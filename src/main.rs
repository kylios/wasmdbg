...
use std::io::{self, Read, BufReader, Cursor, prelude::*};
...
fn main() {
    let ubytes: [u8; 3] = [0xE5, 0x8E, 0x26];
    let mut reader = BufReader::new(Cursor::new(ubytes));
    let uval = u32::parse(&mut reader);
    println!("Unsigned val: {}", uval);
    
    let ibytes: [u8; 3] = [0xc0, 0xbb, 0x78];
    let mut reader = BufReader::new(Cursor::new(ibytes));
    let ival = i32::parse(&mut reader);
    // Should equal -123456
    println!("Signed val: {}", ival);
}
